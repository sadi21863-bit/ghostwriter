...
**`storydiffusion`'s real price, found via a live Playwright fetch of `segmind.com/models/storydiffusion/pricing`** (the static page fetch that worked for the model's parameter schema didn't surface pricing — this needed a rendered-JS scrape): billed at **$0.0072/GPU-second**, with the page showing an estimated **~118.44s** average runtime for this model → **≈$0.85 per call**. Since one `storydiffusion` call generates a whole page of panels (not one panel), this compares directly against the measured $0.29/panel Soul cost: 6 panels × $0.29 = **$1.74/page** today, vs. **≈$0.85/page** if this estimate holds — the opposite of this doc's original caution that ref-sheet/multi-image approaches "get expensive fast." Still needs confirmation via 1-2 real funded calls before rollout (the runtime figure is Segmind's own estimate, not a fixed guarantee, and actual GPU-seconds will vary with panel count) — parked pending the test account being refunded.
