...
## Credential fix: Higgsfield/Segmind key mismatch, 2026-06-21

Verified and fixed a real, pre-launch-blocking credential bug: `src/lib/higgsfield/client.ts` actually talks to **two** separate services — Segmind (`api.segmind.com`, image + video generation, `x-api-key` header) and Higgsfield's own native API (`cloud.higgsfield.ai`, Soul ID character training only, `Authorization: Bearer key:secret`) — but every image/video route was decrypting and passing `user.higgsfieldApiKey` (the wrong credential for Segmind) with Settings labeling that field "Higgsfield AI — API Key, `hf_...`". A user following the UI would paste a Higgsfield key, and every image/video call would fail Segmind auth — while being steered toward buying a Higgsfield subscription (90-day credit expiry) for a feature that actually needs a Segmind key (pay-per-use, no expiry).

Audited every caller of the Segmind-backed client functions (`generateSoulImage`, `generateDoPVideo`, `generateTextVideo`, `generateLipsync`, `pollJob`) and confirmed `soul-id/route.ts` was the only route correctly using `higgsfieldApiKey`/`Secret` (it calls the Higgsfield-native `trainSoulId`/`pollSoulIdTraining`, not Segmind). Every other caller was wrong:

- **Server routes (10):** `audio/lipsync` (×2), `characters/.../portrait`, `comics`, `comics/.../regenerate`, `production/preview-all`, `production/shots/.../animate` (×2: generate + status), `production/shots/.../generate-video` (×2: generate + status), `production/shots/.../preview`. All switched to a new `users.segmindApiKey` column (migration `0010_chubby_the_fallen.sql`).
- **The provider registry itself was part of the problem:** `src/lib/media/adapters/segmind.ts`'s `SegmindImageProvider` was literally named `"Higgsfield Soul 2.0"` — so even the abstraction layer perpetuated the mislabeling in its own display name and in `regenerate/route.ts`'s derived error message. Renamed to `"Segmind (Soul 2.0)"`.
- **Client-side bug beyond the doc's original scope:** found while tracing the fix — `useProjectState.ts` tracked a `higgsfieldKey` UI-gating signal (whether to show generation buttons as enabled) sourced from `higgsfieldKeySet`, threaded through `GhostWriterApp` → `ToolbarPanel` → `ProductionStudioPanel`/`ComicStudioPanel` → `ProductionStudio`/`ComicStudio` (7 files). Since the actual backend calls need a Segmind key, a user with *only* a Segmind key (the expected, recommended common case — Soul ID is optional) would see every generate button disabled even though the backend would have worked. Renamed the whole chain to `segmindKey`/`segmindKeySet`.
- Removed the one `|| process.env.HIGGSFIELD_API_KEY` dev-key fallback (`portrait/route.ts`) — confirmed via grep it was the *only* such fallback in the codebase. Bring-your-own-key for launch; a missing key now returns a clear 400, never a silent fallback to a developer-paid key.
- Fixed every "Add your Higgsfield API key" / generic "API key missing" error message across these paths (server routes, `ComicStudio.tsx`, `ProductionStudio.tsx`, `useWorldBible.ts`) to correctly say "Segmind."

**Settings UI** restructured into two clearly-labeled, captioned sections: "Segmind — image & video generation" (pay-per-use, no expiry, new `segmindApiKey` field, placeholder `SG_...`) and "Higgsfield — character training (Soul ID, optional)" (existing key+secret fields, explicit "skip this if you only want image/video generation" caption).

Also fixed, while addressing the doc's separate manual-setup section (Cloudflare Email Routing, done by Aditya — not a code task): `privacy/page.tsx` and `terms/page.tsx` both referenced `hello@ghostwriterai.com` — a stale, different domain from the actual `ghost-writer.cc`. Switched to `support@ghost-writer.cc` (the landing page's "Contact" link and the email sender's `FROM` default already correctly used `hello@ghost-writer.cc`, left unchanged) — confirmed both addresses are real, Cloudflare-routed-to-Gmail destinations before pointing anything at them, per the doc's explicit "don't invent an address" instruction.

Added a regression test (`portrait/__tests__/route.test.ts`) for the highest-stakes piece — confirms the route uses `segmindApiKey` not `higgsfieldApiKey`, confirms the env-key fallback is gone even when `HIGGSFIELD_API_KEY` is set in the environment, and confirms a missing key returns a clear, correctly-worded 400 rather than failing silently or falling back.
