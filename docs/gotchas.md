...
## A Corrupted `.git/index` Is Not Data Loss — It's a Rebuildable Cache, Not the Commit History

Found 2026-07-17 (item 71): a background `git` process was killed mid-write in a prior session (a `Bash` tool call that hit its timeout), and the next session opened to `git status`/`git add` failing with `fatal: bad signature 0x00000000 / fatal: index file corrupt`. This looks alarming — like the whole repository might be damaged — but `.git/index` is purely the staging-area cache, fully derivable from `HEAD`. `git log` still worked immediately (commit history was never touched), confirming nothing was actually lost.

**Fix**: `rm .git/index && git reset` (a plain, default `git reset` — mixed mode — rebuilds the index from `HEAD` without touching any working-tree file). If the first attempt itself gets killed by a tool timeout (real risk on this project's slow, OneDrive-synced filesystem — a `git reset` scanning many tracked files can genuinely take minutes), it leaves a stale `.git/index.lock` behind; the next attempt fails with "Another git process seems to be running... If it still fails, a git process may have crashed... remove the file manually to continue." Verify no real git process is actually running first (`Get-Process git` in PowerShell returning nothing confirms this), then `rm -f .git/index.lock` and retry — git's own error message states this exact recovery path, it isn't a workaround.

**What was actually at risk**: only uncommitted, *staged-but-not-yet-committed* changes that existed solely in the corrupted index and nowhere else would be lost by this rebuild — anything already committed (`git log`) or still sitting in the working tree as plain files is untouched by rebuilding the index. Confirm via `git log` that history is intact before doing anything else; only then treat the index itself as freely rebuildable.

## Bash Tool's PATH Can Lose POSIX Utils *and* Node/Vercel After a Session Resume

Found 2026-07-17 (item 71): after resuming an interrupted session, every Bash tool call started failing on ordinary commands — `ls`/`head` reported "command not found," and even `vercel` (a real, working global npm install) failed with `sed: command not found` / `node: not found` from inside its own wrapper script. `echo $PATH` showed a bare Windows-style PATH (System32, Program Files\Git\cmd, Program Files\nodejs — but missing `Program Files\Git\usr\bin`, the actual location of Git-Bash's POSIX utils, and with `node`/`vercel`'s directories present but apparently not resolving through this particular shell invocation).

**Fix**: prefix the PATH explicitly on every Bash call for the rest of the session — shell state (unlike the working directory) does not persist between Bash tool invocations, so `export PATH=...` in one call has no effect on the next:
```bash
export PATH="/c/Program Files/Git/usr/bin:/c/Program Files/Git/bin:/c/Program Files/nodejs:/c/Users/aditya/AppData/Roaming/npm:/c/WINDOWS/system32:/c/WINDOWS:$PATH"
```
Prepend this line (adjusted to the real local install paths) to any command in the same session that needs `ls`/`grep`/`sed`/`node`/`git`/`vercel`/etc. and previously worked fine. Dedicated tools (`Read`, `Glob`, `Grep`) are unaffected by this — they don't go through the Bash tool's shell PATH at all, so prefer them over shell equivalents when this gotcha is active and you don't strictly need a real shell command.
