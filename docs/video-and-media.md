...
**Wired into the production pipeline as of item 69** (was previously built but disconnected): `generate-package` fire-and-forget bootstraps and trains a Soul ID for any primary character appearing in 2+ shots via `src/lib/production/soul-id-bootstrap.ts`, never blocking the response, fails open at every step. `getCharacterSoulReference()` prefers a real trained `soulId` over the plain `reference_image_url` fallback once training completes.

**The training connection itself is now live-verified** (item 70, same day): real credentials, real `trainSoulId()`/`pollSoulIdTraining()` call against `platform.higgsfield.ai`, real completed job, real `soulId` returned (~6-7 min end to end) — see `scripts/test-soul-id-connection.mts`. What's **not yet verified**: the full `generate-package` auto-bootstrap path specifically (the account tested there had no keys at all, so it correctly no-op'd via its fail-open path rather than exercising a real run), and using a trained `soulId` to actually generate a new consistent image via `generateSoulImage()` (blocked by the exhausted Segmind balance, a separate provider from the Higgsfield credits the training call used).
