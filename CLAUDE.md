...
10. ✅ Razorpay plans created (6 plans: 3 tiers × monthly+annual), plan IDs in `.env.local`. ✅ Previous ~30%+ "Authentication failed" issue on `subscriptions.create` (3 prior key pairs, unresolved as of 2026-06-11) is RESOLVED as of 2026-06-13 — swapped to a new test key pair (`RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` updated in both `.env` and `.env.local`; `RAZORPAY_WEBHOOK_SECRET` is independent and unchanged). New key pair: `subscriptions.create` succeeded 3/3 then 1/1 with zero retries needed. ✅ Pushed to Vercel production 2026-06-14: `RAZORPAY_KEY_ID`/`RAZORPAY_KEY_SECRET` (new pair) plus the previously-missing `RAZORPAY_CREATOR_PRO_MONTHLY_PLAN_ID`/`RAZORPAY_CREATOR_PRO_ANNUAL_PLAN_ID`. Note: Preview-environment copies of these 4 vars were not updated (Vercel CLI `git_branch_required` issue) — production-only, doesn't block launch.
