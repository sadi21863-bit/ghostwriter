...
export async function GET(_: Request, { params }: Ctx){ const s=await getRequiredSession(); const p=await db.query.projects.findFirst({where:and(eq(projects.id,params.projectId),eq(projects.userId,s.user.id)),with:{chapters:{orderBy:(c,{asc})=>[asc(c.sortOrder)]},characters:true,locations:true,plotThreads:true,referenceWorks:true,storyMemories:true,characterRelationships:true,storyPromises:true}}); if(!p) return NextResponse.json({error:"Not found"},{status:404}); return NextResponse.json(p); }
