...
export async function PATCH(req: Request, { params }: { params: { projectId: string; characterId: string } }) {
  ...
  const [u] = await db.update(characters).set(parsed.data).where(eq(characters.id, params.characterId)).returning();
  return NextResponse.json(u);
}

export async function DELETE(_: Request, { params }: { params: { projectId: string; characterId: string } }) {
  const s = await getRequiredSession();
  if (!await verifyOwnership(params.projectId, s.user.id)) return NextResponse.json({ error: "Not found" }, { status: 404 });
