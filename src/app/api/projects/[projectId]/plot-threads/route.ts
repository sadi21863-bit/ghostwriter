...
export async function POST(req: Request, { params }: { params: Promise<{ projectId: string }> }){
  ...
  const { name, description, status, stakes, connections, alwaysInContext, sortOrder } = await req.json();
  if (!name || typeof name !== "string") return NextResponse.json({ error: "name is required" }, { status: 400 });
  const [r] = await db.insert(plotThreads).values({
    projectId, name, description, status, stakes, connections, alwaysInContext, sortOrder,
  }).returning();
