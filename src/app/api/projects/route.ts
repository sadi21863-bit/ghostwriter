...
export async function POST(req){ const s=await getRequiredSession(); const b=await req.json(); const [p]=await db.insert(projects).values({userId:s.user.id,name:b.name||"Untitled",format:b.format||"Novel",genres:b.genres||[]}).returning(); await db.insert(chapters).values({projectId:p.id,title:"Chapter 1",sortOrder:0}); return NextResponse.json(p,{status:201}); }