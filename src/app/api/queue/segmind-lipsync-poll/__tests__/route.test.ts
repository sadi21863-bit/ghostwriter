...
vi.mock("@/lib/audio/poll-lipsync", () => ({
  pollAndUpdateLipsync: (...args: any[]) => pollAndUpdateLipsync(...args),
}));

import { POST } from "../route";

function makeRequest(body?: any) {
  return new Request("http://localhost/api/queue/segmind-lipsync-poll", {
    method: "POST",
...
describe("POST /api/queue/segmind-lipsync-poll", () => {
  beforeEach(() => {
    vi.clearAllMocks();
  ...
  it("401s when the QStash signature is invalid", async () => {
    ...
    const res = await POST(makeRequest({ userId: "user-1", audioExportId: "a1" }));
  ...
  it("stops without rescheduling when the user has no usable Segmind key", async () => {
    verifyQstashRequest.mockResolvedValue({ userId: "user-1", audioExportId: "a1", attempt: 0 });
  ...
  it("reschedules another poll when the job is still processing and under the attempt cap", async () => {
    verifyQstashRequest.mockResolvedValue({ userId: "user-1", audioExportId: "a1", attempt: 2 });
    pollAndUpdateLipsync.mockResolvedValue({ outcome: "processing" });
    const res = await POST(makeRequest({ userId: "user-1", audioExportId: "a1", attempt: 2 }));
    const body = await res.json();
    expect(body.outcome).toBe("processing");
    expect(scheduleCallback).toHaveBeenCalledWith(expect.objectContaining({
      body: { userId: "user-1", audioExportId: "a1", attempt: 3 },
    }));
  });

  it("does not reschedule once the job reaches completed", async () => {
    verifyQstashRequest.mockResolvedValue({ userId: "user-1", audioExportId: "a1", attempt: 1 });
    pollAndUpdateLipsync.mockResolvedValue({ outcome: "completed", videoUrl: "https://cdn/final.mp4" });
    const res = await POST(makeRequest({ userId: "user-1", audioExportId: "a1", attempt: 1 }));
    const body = await res.json();
  ...
  it("does not reschedule once the attempt cap is reached", async () => {
    verifyQstashRequest.mockResolvedValue({ userId: "user-1", audioExportId: "a1", attempt: 40 });
    pollAndUpdateLipsync.mockResolvedValue({ outcome: "processing" });
    const res = await POST(makeRequest({ userId: "user-1", audioExportId: "a1", attempt: 40 }));
    await res.json();
    expect(scheduleCallback).not.toHaveBeenCalled();
  });
});
